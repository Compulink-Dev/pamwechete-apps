...
tradeSchema.statics.calculateTradePoints = function(valuation, condition, category) {
  ...
  const years = age / 12;
  const ageDepreciation = years <= 5 ? (baseValue * 0.05 * years) : (baseValue * 0.25);
  ...
  const points = (baseValue * conditionMultiplier * qualityMultiplier * demandBonus) - ageDepreciation;
...
tradeSchema.pre('save', function(next) {
  if (this.isModified('valuation') || this.isModified('condition') || this.isNew) {
